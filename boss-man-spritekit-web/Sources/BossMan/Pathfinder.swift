...
// BFS shortest-path planner. Same chase shape as bossman-apple's Pathfinder,
// but tuned for the wasm single thread: the apple port allocated a
// Dictionary per call plus a result array (and a direction-tuple literal) for
// every node visited. On the 6-boss levels (12, 24) those allocations
// clustered into frame spikes and the hero stuttered. This version reuses flat
// scratch buffers keyed by cell index with a generation stamp, so a full BFS
// allocates nothing but the returned path. The neighbour visit order is kept
// identical to GridMap.walkableNeighbors (right, left, up, down, then tunnel)
// so ties break the same way and the chosen path matches the master exactly.
