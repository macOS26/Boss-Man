...
open class SKNode {
    ...
    public func setScale(_ s: CGFloat) { xScale = s
    yScale = s }

    open func addChild(_ node: SKNode) { node.parent = self
    children.append(node) }
    public func insertChild(_ node: SKNode, at index: Int) { node.parent = self
    ...
    public func removeAllChildren() { for c in children { c.parent = nil
    c.teardownPhysics() }
    children.removeAll() }
    ...
    public func inParentHierarchy(_ candidate: SKNode) -> Bool {
        ...
        while let p = n { if p === candidate { return true }
        n = p.parent }
        return false
    }
    ...
    final func stepActions(_ dt: CGFloat) {
        ...
        while i < runningActions.count {
            ...
            guard stepped.insert(ObjectIdentifier(ra)).inserted else { i += 1
            continue }
