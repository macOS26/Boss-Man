...
public extension SKNode {
    ...
    func absolutePosition() -> CGPoint {
        ...
        while let cur = n { p = CGPoint(x: p.x + cur.position.x, y: p.y + cur.position.y); n = cur.parent }
        return p
    }
    func convert(_ point: CGPoint, from node: SKNode) -> CGPoint {
        let a = node.absolutePosition(); let me = absolutePosition()
        return CGPoint(x: point.x + a.x - me.x, y: point.y + a.y - me.y)
    }
    func convert(_ point: CGPoint, to node: SKNode) -> CGPoint {
        let me = absolutePosition(); let other = node.absolutePosition()
...
public extension SKAction {
    ...
    static func colorize(with color: SKColor, colorBlendFactor: CGFloat, duration d: TimeInterval) -> SKAction { .wait(forDuration: d) }
    ...
    func reversed() -> SKAction { self }   // best-effort: most game actions are symmetric or replayed
}
