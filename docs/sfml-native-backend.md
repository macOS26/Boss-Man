...
- `Box2DBridge` (in `WasmKit/spritekit/Sources/`) already proves C interop works for Box2D. The same bridge pattern applies to CSFML.
- The Canvas 2D backend (`WasmKit/spritekit/`) defines the full SpriteKit API surface the game uses. That API is the contract the SFML backend must satisfy.
...
Add CSFML as a system library target in `WasmKit/spritekit/Package.swift` (or as a vendored source target). Create a thin Swift overlay (`SFMLKit`) that wraps:
...
- `WasmKit/spritekit/` — Canvas 2D backend (the API contract to match)
